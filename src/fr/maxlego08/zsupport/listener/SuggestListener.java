...
public class SuggestListener extends ListenerAdapter implements Constant {
	...
	public void onMessageReceived(MessageReceivedEvent event) {
		...
		message.getChannel().sendMessage(new EmbedBuilder()
				...
				.setColor(Color.getHSBColor(5, 255, 5)).setDescription(message.getContentDisplay())
				.setFooter("2021 - " + guild.getName(), guild.getIconUrl()).build()).queue(discordMessage -> {

					discordMessage.addReaction("U+2705").queue(e -> {						
						discordMessage.addReaction("U+274C").queue(e2 -> {						
